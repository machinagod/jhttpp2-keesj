...
public class Jhttpp2Admin {
						...
						if (filterurl != null && action != null) {
							Jhttpp2URLMatch a = new Jhttpp2URLMatch(filterurl,
									false/* set to real */, Integer
											.parseInt(action), ""/* set to real! */); // add
																						// new
																						// url
																						// match
	...
	public String HTMLAdmin() {
		...
		if (server.config_auth == 0) {
			...
			msg = msg
					...
					+ chk_option(
							"server.webconfig <br><i>Enables the web-based configuration module</I>",
							"webconfig", server.webconfig)
					+ chk_option(
							"server.www <br><i> The built-in webserver </i>",
							"www_server", server.www_server)
					// + chk_option ("server.debug-logging" , "debug",
					// server.debug)
					+ chk_option("server.debug-logging", "debug", server.debug)
					+ chk_option("server.http-proxy", "use_proxy",
							server.use_proxy)
					+ chk_option("server.fiter.http", "filter_http",
							server.filter_http)
					...
					+ txt_field("", "server.port", "port", Integer
							.toString(server.port))
					+ chk_option("server.access.log", "log_access",
							server.log_access)
					+ txt_field("", "server.access.log.filename",
							"log_access_filename", server.log_access_filename)
					// +txt_field ("server.webconfig.username",
					// "config_username", server.config_user)
					// +txt_field ("server.webconfig.password",
					// "config_password", server.config_password,true)
					+ "<tr>"
					...
					+ "\">Logout</a>&nbsp;<a href=\"/" + server.WEB_CONFIG_FILE
		...
		msg = msg
				...
				+ "<p class=\"status\">" + status + "</p>\r" + "</BODY></HTML>";
	...
	public static String chk_option(String o, String o2, boolean w) {
		return "<tr>\r" + "<td width=\"52\" height=\"6\">\r" + "<input name=\""
				+ o2 + "\" type=\"checkbox\"" + (w ? " checked " : "")
				+ "/></td>\r" + "<td width=\"449\" height=\"6\">" + o
				+ "</td>\r" + "</tr>\r";
	}
